...
class BusinessNewsPage extends StatefulWidget {
  ...
  final ApiClient? api;
  const BusinessNewsPage({Key? key,this.api}) : super(key: key);
...
class _BusinessNewsPageState extends State<BusinessNewsPage> {
  ...
  @override
  void initState() {
    store.getBusinessNews(widget.api!);
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Observer(
      builder: (_) {
        final businessNews = store.businessNewsList?.status;
        switch (businessNews) {
          ...
          case FutureStatus.fulfilled:
            List<NewsModel> newsList = store.businessNewsList?.result ?? [];
